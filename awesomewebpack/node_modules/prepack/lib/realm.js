...
exports.Realm = exports.ExecutionContext = exports.Tracer = exports.Effects = undefined;
...
function construct_empty_effects(realm, c = new _completions.SimpleNormalCompletion(realm.intrinsics.empty, undefined)) {
...
class Realm {
  constructor(opts, statistics) {
    ...
    this.suppressDiagnostics = false;
    this.objectCount = 0;
  ...
  clearBlockBindingsFromCompletion(completion, environmentRecord) {
    if (completion instanceof _completions.PossiblyNormalCompletion) {
      ...
      if (completion.savedEffects !== undefined) this.clearBlockBindings(completion.savedEffects.modifiedBindings, environmentRecord);
    ...
    } else if (completion instanceof _completions.ForkedAbruptCompletion) {
      this.clearBlockBindings(completion.alternateEffects.modifiedBindings, environmentRecord);
      this.clearBlockBindings(completion.consequentEffects.modifiedBindings, environmentRecord);
      if (completion.alternate instanceof _completions.Completion) this.clearBlockBindingsFromCompletion(completion.alternate, environmentRecord);
      if (completion.consequent instanceof _completions.Completion) this.clearBlockBindingsFromCompletion(completion.consequent, environmentRecord);
  ...
  clearFunctionBindingsFromCompletion(completion, funcVal) {
    if (completion instanceof _completions.PossiblyNormalCompletion) {
      ...
      if (completion.savedEffects !== undefined) this.clearFunctionBindings(completion.savedEffects.modifiedBindings, funcVal);
  ...
  _getCheckedBindings() {
    ...
    (0, _invariant2.default)(globalObject instanceof _index.ObjectValue);
    ...
    (0, _invariant2.default)(checkedBindingsObject instanceof _index.ObjectValue);
    return checkedBindingsObject;
  }

  markPropertyAsChecked(object, P) {
    (0, _invariant2.default)(!this.neverCheckProperty(object, P));
  ...
  // call.
  evaluatePure(f, reportSideEffectFunc) {
  ...
  withEffectsAppliedInGlobalEnv(func, effects) {
    ...
    this.evaluateForEffectsInGlobalEnv(() => {
      try {
        ...
        (0, _invariant2.default)(!effects.canBeApplied);
  ...
  evaluateWithUndoForDiagnostic(f) {
    ...
    try {
      ...
      (0, _invariant2.default)(resultVal instanceof _completions.SimpleNormalCompletion);
    ...
    try {
      ...
      while (true) {
        ...
        if (_singletons.Widen.containsEffects(effects1, effects2)) {
          ...
          (0, _invariant2.default)(test instanceof _index.AbstractValue);
  ...
  evaluateWithAbstractConditional(condValue, consequentEffectsFunc, alternateEffectsFunc) {
    ...
    if (effects1 === undefined || effects2 === undefined) {
      ...
      joinedEffects = _singletons.Join.joinForkOrChoose(this, condValue, effects1, effects2);
      completion = joinedEffects.result;
      if (completion instanceof _completions.ForkedAbruptCompletion) {
  ...
  _emitPropertAssignments(gen, pbindings, newlyCreatedObjects) {
    ...
    pbindings.forEach((val, key, map) => {
      ...
      if (value instanceof _index.AbstractValue) {
        ...
        let tval = gen.deriveAbstract(value.types, value.values, [key.object, value], ([o, n]) => {
      ...
      if (typeof keyKey === "string") {
        if (path !== undefined) {
          gen.emitStatement([key.object, tval || value, this.intrinsics.empty], ([o, v, e]) => {
            ...
            (0, _invariant2.default)(typeof keyKey === "string");
        ...
        (0, _invariant2.default)(keyKey instanceof _index.Value);
  ...
  composeEffects(priorEffects, subsequentEffects) {
    ...
    result.generator = _singletons.Join.composeGenerators(this, priorEffects.generator || result.generator, subsequentEffects.generator);
    ...
    if (priorEffects.modifiedProperties) {
      priorEffects.modifiedProperties.forEach((desc, propertyBinding, m) => result.modifiedProperties.set(propertyBinding, desc));
    }
  ...
  wrapSavedCompletion(completion) {
  ...
  composeWithSavedCompletion(completion) {
    if (this.savedCompletion === undefined) {
      ...
      savedCompletion = _singletons.Join.composePossiblyNormalCompletions(this, savedCompletion, completion, e);
      this.applyEffects(e);
    ...
    function pushPathConditionsLeadingToNormalCompletion(c) {
      if (allPathsAreAbrupt(c.consequent)) {
        ...
        if (c.alternate instanceof _completions.PossiblyNormalCompletion || c.alternate instanceof _completions.ForkedAbruptCompletion) pushPathConditionsLeadingToNormalCompletion(c.alternate);
      } else if (allPathsAreAbrupt(c.alternate)) {
        _singletons.Path.pushAndRefine(c.joinCondition);
        if (c.consequent instanceof _completions.PossiblyNormalCompletion || c.consequent instanceof _completions.ForkedAbruptCompletion) pushPathConditionsLeadingToNormalCompletion(c.consequent);
        ...
        if (!allPathsAreNormal(c.alternate)) {
          ...
          let disjunct = _index.AbstractValue.createFromLogicalOp(realm, "||", c.joinCondition, alternatePC);
        ...
        let inverse = _index.AbstractValue.createFromUnaryOp(realm, "!", c.joinCondition);
        let disjunct = _index.AbstractValue.createFromLogicalOp(realm, "||", inverse, consequentPC);
        _singletons.Path.pushAndRefine(disjunct);
      } else {
        let jc = c.joinCondition;
        let consequentPC = _index.AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditionFor(c.consequent));
        let ijc = _index.AbstractValue.createFromUnaryOp(realm, "!", jc);
        let alternatePC = _index.AbstractValue.createFromLogicalOp(realm, "&&", ijc, getNormalPathConditionFor(c.alternate));
        let disjunct = _index.AbstractValue.createFromLogicalOp(realm, "||", consequentPC, alternatePC);
        _singletons.Path.pushAndRefine(disjunct);
      }
    }

    function allPathsAreAbrupt(c) {
      if (c instanceof _completions.ForkedAbruptCompletion) return allPathsAreAbrupt(c.consequent) && allPathsAreAbrupt(c.alternate);
      if (c instanceof _completions.AbruptCompletion) return true;
      return false;
    }

    function allPathsAreNormal(c) {
      if (c instanceof _completions.PossiblyNormalCompletion || c instanceof _completions.ForkedAbruptCompletion) return allPathsAreNormal(c.consequent) && allPathsAreNormal(c.alternate);
      if (c instanceof _completions.AbruptCompletion) return false;
      return true;
    }

    function getNormalPathConditionFor(c) {
      (0, _invariant2.default)(c instanceof _completions.PossiblyNormalCompletion || c instanceof _completions.ForkedAbruptCompletion);
      if (allPathsAreAbrupt(c.consequent)) {
        (0, _invariant2.default)(!allPathsAreAbrupt(c.alternate));
        let inverse = _index.AbstractValue.createFromUnaryOp(realm, "!", c.joinCondition);
        if (allPathsAreNormal(c.alternate)) return inverse;
        return _index.AbstractValue.createFromLogicalOp(realm, "&&", inverse, getNormalPathConditionFor(c.alternate));
      } else if (allPathsAreAbrupt(c.alternate)) {
        (0, _invariant2.default)(!allPathsAreAbrupt(c.consequent));
        if (allPathsAreNormal(c.consequent)) return c.joinCondition;
        return _index.AbstractValue.createFromLogicalOp(realm, "&&", c.joinCondition, getNormalPathConditionFor(c.consequent));
      } else if (allPathsAreNormal(c.consequent)) {
        // In principle the simplifier shoud reduce the result of the else clause to this case. This does less work.
        (0, _invariant2.default)(!allPathsAreNormal(c.alternate));
        (0, _invariant2.default)(!allPathsAreAbrupt(c.alternate));
        let ijc = _index.AbstractValue.createFromUnaryOp(realm, "!", c.joinCondition);
        let alternatePC = _index.AbstractValue.createFromLogicalOp(realm, "&&", ijc, getNormalPathConditionFor(c.alternate));
        return _index.AbstractValue.createFromLogicalOp(realm, "||", c.joinCondition, alternatePC);
      } else if (allPathsAreNormal(c.alternate)) {
        // In principle the simplifier shoud reduce the result of the else clause to this case. This does less work.
        (0, _invariant2.default)(!allPathsAreNormal(c.consequent));
        (0, _invariant2.default)(!allPathsAreAbrupt(c.consequent));
        let jc = c.joinCondition;
        let consequentPC = _index.AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditionFor(c.consequent));
        let ijc = _index.AbstractValue.createFromUnaryOp(realm, "!", jc);
        return _index.AbstractValue.createFromLogicalOp(realm, "||", consequentPC, ijc);
      } else {
        let jc = c.joinCondition;
        let consequentPC = _index.AbstractValue.createFromLogicalOp(realm, "&&", jc, getNormalPathConditionFor(c.consequent));
        let ijc = _index.AbstractValue.createFromUnaryOp(realm, "!", jc);
        let alternatePC = _index.AbstractValue.createFromLogicalOp(realm, "&&", ijc, getNormalPathConditionFor(c.alternate));
        return _index.AbstractValue.createFromLogicalOp(realm, "||", consequentPC, alternatePC);
      }
    }
  }
  ...
  captureEffects(completion) {
    (0, _invariant2.default)(completion.savedEffects === undefined);
  ...
  getCapturedEffects(v = this.intrinsics.undefined) {
    ...
    (0, _invariant2.default)(this.modifiedProperties !== undefined);
    (0, _invariant2.default)(this.createdObjects !== undefined);
    return new Effects(new _completions.SimpleNormalCompletion(v), this.generator, this.modifiedBindings, this.modifiedProperties, this.createdObjects);
  }
  ...
  applyEffects(effects, leadingComment = "", appendGenerator = true) {
    (0, _invariant2.default)(effects.canBeApplied, "Effects have been applied and not properly reverted. It is not safe to apply them a second time.");
  ...
  recordModifiedBinding(binding, value) {
    const isDefinedInsidePureFn = root => {
      ...
      (0, _invariant2.default)(func instanceof _index.FunctionValue);
      if (root instanceof _environment.FunctionEnvironmentRecord && func === root.$FunctionObject) {
        return true;
      }
    ...
    if (this.modifiedBindings !== undefined && !this.modifiedBindings.has(binding) && value !== undefined && this.isInPureScope() && this.reportSideEffectCallback !== undefined) {
  ...
  rebuildObjectProperty(object, key, propertyValue, path) {
    ...
    if (!propertyValue.isIntrinsic()) {
      ...
      propertyValue._buildNode = ([node]) => t.isValidIdentifier(key) ? t.memberExpression(node, t.identifier(key), false) : t.memberExpression(node, t.stringLiteral(key), true);
  ...
  createErrorThrowCompletion(type, message) {
    ...
    (0, _invariant2.default)(message instanceof _index.StringValue);
    this.nextContextLocation = this.currentLocation;
    return new _completions.ThrowCompletion((0, _index2.Construct)(this, type, [message]), undefined, this.currentLocation);
  }
    ...
    return this.evaluateForEffects(() => {
      ...
      try {
        ...
        for (let priorEffect of priorEffects) {
          (0, _invariant2.default)(!priorEffect.canBeApplied);
  ...
  getTemporalBuildNodeEntryFromDerivedValue(value) {
  ...
  saveTemporalGeneratorEntryArgs(temporalBuildNodeEntry) {
