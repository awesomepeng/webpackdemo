...
var _JSONTokenizer = require("./utils/JSONTokenizer.js");

var _JSONTokenizer2 = _interopRequireDefault(_JSONTokenizer);
    ...
    if (!arg.startsWith("--")) {
      ...
      switch (arg) {
        ...
        case "debugDiagnosticSeverity":
          arg = args.shift();
          (0, _invariant2.default)(arg === "FatalError" || arg === "RecoverableError" || arg === "Warning" || arg === "Information", `Invalid debugger diagnostic severity: ${arg}`);
        ...
        case "debugBuckRoot":
          ...
          break;
        case "help":
          const options = ["-- | input.js", "--out output.js", "--compatibility jsc", "--mathRandomSeed seedvalue", "--srcmapIn inputMap", "--srcmapOut outputMap", "--maxStackDepth depthValue", "--timeout seconds", "--debugIdentifiers id1,id2,...", "--check [start[, number]]", "--lazyObjectsRuntime lazyObjectsRuntimeName", "--heapGraphFilePath heapGraphFilePath", "--reactOutput " + _options.ReactOutputValues.join(" | "), "--repro reprofile.zip", "--cpuprofile name.cpuprofile", "--invariantMode " + _options.InvariantModeValues.join(" | ")];
...
`);
  ...
  function printDiagnostics(caughtFatalError) {
    if (compilerDiagnostics.size === 0 && compilerDiagnosticsList.length === 0) {
      ...
      (0, _invariant2.default)(!caughtFatalError, "FatalError must generate at least one CompilerDiagnostic");
      return true;
    }
    ...
    let printCompilerDiagnostic = (compilerDiagnostic, locString = "At an unknown location") => {
      switch (compilerDiagnostic.severity) {
        ...
        case "RecoverableError":
          ...
          break;
        default:
          (0, _invariant2.default)(compilerDiagnostic.severity === "FatalError");
          fatalErrors++;
          break;
      }
    ...
    (0, _invariant2.default)(informations + warnings + recoverableErrors + fatalErrors > 0);
    let plural = (count, word) => count === 1 ? word : `${word}s`;
    console.error(`Prepack ${fatalErrors > 0 ? "failed" : "succeeded"}, reporting ${[fatalErrors > 0 ? `${fatalErrors} ${plural(fatalErrors, "fatal error")}` : undefined, recoverableErrors > 0 ? `${recoverableErrors} ${plural(recoverableErrors, "recoverable error")}` : undefined, warnings > 0 ? `${warnings} ${plural(warnings, "warning")}` : undefined, informations > 0 ? `${informations} ${plural(informations, "informational message")}` : undefined].filter(s => s !== undefined).join(", ")}.`);
  ...
  try {
    ...
    if (profiler !== undefined) {
      ...
      let start = Date.now();
      (0, _invariant2.default)(cpuprofilePath !== undefined);
      let stream = fs.createWriteStream(cpuprofilePath);
      let getNextToken = (0, _JSONTokenizer2.default)(data);
      let write = () => {
